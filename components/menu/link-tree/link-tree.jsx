...
import classes from './link-tree.module.css';

function handleToggle(links, name) {
  const newLinks = [...links];
  const targetElementIdx = newLinks.findIndex((linkElement) => linkElement.name === name);
  ...
  for (let i = targetElementIdx + 1; i < newLinks.length; i++) {
    ...
    if (targetCurrentIsToggled) {
      ...
      if (currentDepth === targetDepth + 1 && 'isToggled' in currentElement) currentElement.isToggled = false;
    } else {
...
function LinkTree({ toggleMenu }) {
  ...
  return (
    <div className={classes.linkTree}>
      {
          ...
          <div key={index} style={{paddingLeft: `${40 * link.depth}px`}}>
            {
              'isToggled' in link ?
                <ToggleLink
                  linkElement={link}
                  biggerShift={link.isToggled && links[index + 1].type === 'link'}
